    ...
    return (
        <div className='bg-gray-900 pt-20 h-full lg:h-screen'>
            ...
            {data?.tag.map(item=>(
                ...
                <Link to={(`/tag/${encodeURIComponent(item.slug)}/?ordering=-popular&`)} className='text-indigo-400 my px-1 
            ...
            {data?.tags.map(item=>(
                <Link  to={(`/tag/${encodeURIComponent(item.slug)}/?ordering=-popular&`)} className='m'>
                    <li >
                    {item.title}
                    </li>
                </Link>
