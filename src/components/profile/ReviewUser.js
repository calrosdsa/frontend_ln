...
function ReviewUser() {
    const {data,mutate} = useSWR('https://light-nvls.herokuapp.com/profile/review', fetcher)
    return (
        <div className='bg-gray-900   pt-20  pb-56'>
          ...
          <h1 className='text-2xl  text-gray-400'>Recent Reviews Last 20</h1>
            {data?.reviews.map(item=>(
             ...
             <h1 className="w-full my-1 pb-1 border-b-2 border-gray-400   text-xs sm:text-sm lg:text-base  text-white">{item?.review}</h1>
