...
export const getHistory=()=> async (dispatch) =>{
  ...
  try {
    const res = await axios.get(`/profile/detail/`)
    dispatch({
      type: GET_HISTORY,
      payload:res.data
    })
...
export const getLibrary = () => async (dispatch) => {
  ...
  try {
    const res = await axios.get(`/novels/library`);
    dispatch({
      type : GET_LIBRARY,
      payload: res.data.profile,
    });
...
export const addToLibrary=(id)=> async(dispatch)=>{
  if (localStorage.token){
    setAuthToken( localStorage.token);
  }

  try{
    const res = await axios.post(`/novels/favorites-products/update/${id}/`)
    dispatch({
      type : ADD_TO_LIBRARY,
      payload : {library: res.data}
    });
  }catch (err) {
    dispatch({
      type: PROFILE_ERROR,
...
export const getProfiles = () => async (dispatch) => {
  dispatch({ type: CLEAR_MEMBERPROFILE }); // we need clear profile here so 
  //that the previous user's profile doesnt get remained in the browser
